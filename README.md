...
Это starter проект со слабыми детерминированными guardrail и tester для опубликованной синтетической Trust & Safety policy. Меняйте внутреннюю реализацию guardrail, не нарушая HTTP-контракт, и измеряйте результат воспроизводимым способом.

Базовый guardrail — гибрид двух детерминированных механизмов: упорядоченных
...
Векторной БД, embedding-модели и внешних вызовов нет.

Векторная часть настроена слабо – в starter-каталоге мало прототипов,
пороги фиксированы, `message` и `evidence` перед детекторами склеиваются, а
vector signal покрывает только четыре `BLOCK`-семейства. Вы может улучшать
